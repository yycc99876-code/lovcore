...
export function InlineAICommand({ editor, containerRef }: InlineAICommandProps) {
  ...
  const syncSelection = useCallback(() => {
    if (panelOpen || pendingDiff || status === 'thinking') return;
    const current = readSelection();
    setSelection(current);
    if (current) {
      setCompactOpen(true);
  ...
  }, [panelOpen, pendingDiff, readSelection, status]);
  ...
  useEffect(() => {
    const onPointerDown = (event: PointerEvent) => {
      ...
      if (diffRef.current?.contains(target)) return;
  ...
  }, [closeAll, compactOpen, containerRef, panelOpen, pendingDiff, status]);
  ...
  return (
    <>
      {compactOpen && selection && !panelOpen && !pendingDiff && status !== 'thinking' && (
        ...
        >
          <LogoIcon size={18} className="inline-ai-compact-logo" />
          <input
            ref={compactInputRef}
            value={commandText}
        ...
        >
          <div className="inline-ai-panel-header">
            <div className="inline-ai-panel-title">
              <LogoIcon size={19} className="inline-ai-panel-logo" />
