...
interface LovcoreEditorProps {
  ...
  autoFocus?: boolean;
  enableGhost?: boolean;
  voiceDisplay?: 'immersive' | 'compact';
}
...
export const LovcoreEditor = ({
  ...
  useEffect(() => {
    ...
    el.addEventListener('lovcore:insert-image-file', onInsertImage);
    return () => el.removeEventListener('lovcore:insert-image-file', onInsertImage);
  ...
  useEffect(() => {
    ...
    const onDrop = (event: DragEvent) => {
      const image = getImageFile(event.dataTransfer?.files);
      if (!image) return;
      event.preventDefault();
    ...
    const onPaste = (event: ClipboardEvent) => {
      const image = getImageFile(event.clipboardData?.files);
      if (!image) return;
      event.preventDefault();
      insertImageFile(image);
    };
