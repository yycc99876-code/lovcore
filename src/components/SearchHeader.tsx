...
export type VaultView = 'stack' | 'spaces' | 'serendipity' | 'folio-detail';
  ...
  const renderTopNav = (compact = false) => (
    <nav className={compact ? 'vault-nav compact' : 'vault-nav'} aria-label="Lovcore views">
      {(['stack', 'spaces', 'serendipity'] as VaultView[]).map((view) => (
        <button
          key={view}
          className={`vault-nav-btn ${activeView === view || (view === 'spaces' && activeView === 'folio-detail') ? 'active' : ''}`}
        ...
        <div className="sticky-header-content">
          ...
          {activeView === 'stack' ? (
            ...
            <div className="compact-section-title">
              {activeView === 'spaces' || activeView === 'folio-detail' ? t.header.folios : t.header.echoes}
            </div>
          )}
